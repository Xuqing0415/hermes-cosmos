...
def _lazy_import():
    ...
    from hermes_unified.optimizations import GradientCompressor as GC
    from hermes_unified.optimizations import GradientFusion as GF
        ...
        if self.use_compression:
            for name, param in self.model.named_parameters():
                if param.grad is not None:
                    compressed, meta = self.gradient_compressor.compress(param.grad, name)
...
def run_adaptive_training_example():
    ...
    for opt, enabled in status.items():
        print(f"  {opt}: {'[OK] 启用' if enabled else '[FAIL] 禁用'}")
