        ...
        for step in range(self.config.simulation_steps):
            ...
            if step % self.config.switch_interval == 0:
                ...
                if best_protocol != self.current_protocol:
                    ...
                    print(
                        f"  Throughputs: PS={all_throughputs['ps']:.1f}, "
                        f"DDP={all_throughputs['ddp']:.1f}, Gossip={all_throughputs['gossip']:.1f}"
                    )
...
def print_results(results: Dict):
    ...
    for r in results["results"]:
        if current_protocol is None:
            ...
            switches.append(
                f"  Step {start_step}-{r['step']-1}: {current_protocol.upper()} ({r['step']-start_step} steps)"
            )
            current_protocol = r["protocol"]
            start_step = r["step"]

    if current_protocol:
        last_step = results["results"][-1]["step"]
        switches.append(
            f"  Step {start_step}-{last_step}: {current_protocol.upper()} ({last_step - start_step + 1} steps)"
        )
...
def plot_results(results: Dict):
    ...
    for r in results["results"]:
        ax2.scatter(r["step"], protocol_y[r["protocol"]], color=protocol_colors[r["protocol"]], s=50)
