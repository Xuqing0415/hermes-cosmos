...
class BayesianOptimizer:
    ...
    def __init__(self, objective_func: Callable, search_space: List[Tuple], n_calls: int = 20, acq_func: str = "EI"):
        """
        Args:
        ...
        # 高斯过程模型
        self.gp = GaussianProcessRegressor(kernel=Matern(nu=2.5), n_restarts_optimizer=10, random_state=42)

        print(f"[START] BayesianOptimizer initialized with {n_calls} calls")

    def _normalize_params(self, params: List[float]) -> List[float]:
        """归一化参数到 [0, 1] 范围"""
        normalized = []
        for i, (_name, p_type, low, high) in enumerate(self.search_space):
            if p_type == "integer":
    ...
    def suggest(self) -> Dict:
        ...
        # 转换为字典格式
        result = {}
        for i, (name, _p_type, _low, _high) in enumerate(self.search_space):
    ...
    def tell(self, params: Dict, loss: float):
        ...
        # 转换为列表并归一化
        normalized = []
        for name, _p_type, low, high in self.search_space:
            normalized.append((params[name] - low) / (high - low))

        self.X.append(normalized)
        self.y.append(loss)

        print(f"[STATS] Trial {len(self.y)}: {params} -> loss={loss:.4f}")

    def run(self) -> Tuple[Dict, float]:
        """运行完整优化过程"""
        print("\n[TUNE] Starting Bayesian Optimization...")
