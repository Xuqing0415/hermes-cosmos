...
# 尝试导入 Hermes 模块
try:
    # 只为探测依赖是否可用（下面只用到 HERMES_AVAILABLE），名字本身不参与计算
    from hermes_unified.core.parameter_server import RaftParameterServer  # noqa: F401
    from hermes_unified.core.worker import Worker  # noqa: F401
...
class MetricsRecorder:
    ...
    def record_batch(
        ...
        param_change_rate: float = None,
        comm_delay_ms: float = None,
    ):
        """记录每批次指标"""
        self.batch_count += 1
    ...
    def record_epoch(self, epoch: int, throughput: float, epoch_time: float, accuracy: float):
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
            (self.run_id, self.mode, epoch, throughput, epoch_time, accuracy, datetime.now().isoformat()),
        )
        self.conn.commit()
...
def train_with_amp(model, train_loader, criterion, optimizer, device, scaler, metrics_recorder, epoch, args):
    """使用 AMP 训练一个 epoch"""
    model.train()
    total_loss = 0.0
    total_samples = 0
...
def benchmark_ddp(args):
    ...
    for epoch in range(args.epochs):
        ...
        throughput, epoch_time, avg_loss = train_with_amp(
            model, train_loader, criterion, optimizer, device, scaler, metrics_recorder, epoch, args
        )
        ...
        results.append({"epoch": epoch, "throughput": throughput, "epoch_time": epoch_time, "accuracy": accuracy})
...
def benchmark_hermes(args, mode="gossip"):
    ...
    if not HERMES_AVAILABLE:
        print("[WARN] Hermes modules not available, skipping...")
        return None
...
def run_benchmark(args):
    ...
    with open(output_file, "w") as f:
        json.dump({"timestamp": datetime.now().isoformat(), "args": vars(args), "results": all_results}, f, indent=2)
    ...
    for result in all_results:
        print(
            f"{result['mode']:<20} {result['avg_throughput']:<15.2f} "
...
def main():
    ...
    # 模型参数
    parser.add_argument("--depth", type=int, default=18, choices=[18, 50], help="ResNet depth (18 or 50)")
    parser.add_argument(
        "--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"], help="Dataset to use"
    )
    ...
    # 运行参数
    parser.add_argument(
        "--mode", type=str, default="all", choices=["ddp", "gossip", "ps", "all"], help="Benchmark mode"
    )
