...
class GraphDataGenerator:
    ...
    def generate_barabasi_albert(
        self, num_nodes: int, num_attachments: int = 3, feature_dim: int = 16, num_classes: int = 3
    ) -> GraphDataset:
        """
        ...
        for new_node in range(num_attachments, num_nodes):
            ...
            if total_degree == 0:
                ...
                targets = self.rng.choice(new_node, min(num_attachments, new_node), replace=False, p=probs)
    ...
    def generate_sbm(
        self, num_nodes: int, num_communities: int = 3, p_in: float = 0.3, p_out: float = 0.05, feature_dim: int = 16
    ) -> GraphDataset:
        """
    ...
    def _generate_node_labels(self, node_features: np.ndarray, num_classes: int) -> np.ndarray:
...
class HorizontalGraphPartitioner:
    ...
    def generate_molecular_graphs(
        self, num_graphs: int, num_clients: int, feature_dim: int = 10, seed: int = 42
    ) -> Dict[int, List[GraphDataset]]:
        """
...
class VerticalGraphPartitioner:
    ...
    def partition(
        self, graph: GraphDataset, num_clients: int, partition_strategy: str = "random", overlap_ratio: float = 0.0
    ...
    def _degree_partition(self, adj_matrix: np.ndarray, num_clients: int) -> np.ndarray:
    ...
    def _extract_local_edges(self, adj_matrix: np.ndarray, local_nodes: np.ndarray) -> np.ndarray:
    ...
    def get_cross_edge_statistics(self, cross_edge_info: Dict[int, Dict]) -> Dict[str, Any]:
        """Compute statistics about cross-client edges."""
        total_cross_edges = sum(info["num_cross_edges"] for info in cross_edge_info.values())

        cross_edge_pairs = {}
        for client_id, info in cross_edge_info.items():
            for _src, _dst, dst_client in info["cross_edges"]:
    ...
    def create_overlap_partition(
        self, graph: GraphDataset, num_clients: int, overlap_ratio: float = 0.1
    ) -> Tuple[Dict[int, GraphDataset], Dict[str, Any]]:
        """
        ...
        for node in overlap_nodes:
            ...
            extra_clients = self.rng.choice(other_clients, min(2, len(other_clients)), replace=False)
        ...
        for client_id in range(num_clients):
            ...
            overlap_nodes_for_client = [
                n
                for n, clients in overlap_assignments.items()
                if client_id in clients and base_assignments[n] != client_id
            ]

            local_nodes = (
                np.concatenate([primary_nodes, np.array(overlap_nodes_for_client, dtype=np.int64)])
                if overlap_nodes_for_client
                else primary_nodes
            )
...
def create_synthetic_graph_dataset(
    ...
    for i in range(num_graphs):
        if dataset_type == "community":
            graph = generator.generate_sbm(num_nodes=num_nodes, num_communities=num_classes, feature_dim=feature_dim)
        elif dataset_type == "scale_free":
            graph = generator.generate_barabasi_albert(
                num_nodes=num_nodes, num_attachments=3, feature_dim=feature_dim, num_classes=num_classes
            )
        else:
            graph = generator.generate_erdos_renyi(
                num_nodes=num_nodes, edge_prob=0.1, feature_dim=feature_dim, num_classes=num_classes
            )
...
if __name__ == "__main__":
    ...
    print("\n1. Generating Erdos-Renyi graph...")
    er_graph = generator.generate_erdos_renyi(num_nodes=50, edge_prob=0.1, feature_dim=16, num_classes=3)
    ...
    sbm_graph = generator.generate_sbm(num_nodes=100, num_communities=4, p_in=0.3, p_out=0.05, feature_dim=16)
    ...
    h_client_data = h_partitioner.partition(
        num_graphs=20, num_clients=4, graph_generator=generator, min_nodes=20, max_nodes=50, non_iid=True
    )
    ...
    v_client_graphs, v_info = v_partitioner.partition(graph=sbm_graph, num_clients=3, partition_strategy="random")
