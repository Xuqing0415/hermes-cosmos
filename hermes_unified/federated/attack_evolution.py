...
class AttackEvolution:
    ...
    def _random_individual(self):
        ...
        return {
            ...
            "trigger_pos": (
                (random.randint(0, 31), random.randint(0, 31)) if random.random() > 0.5 else None
            ),  # 后门触发位置
        }
    ...
    def _fitness(self, individual):
        ...
        try:
            ...
            results = sim.run_federated_training(num_rounds=30, clients_per_round=5, local_epochs=2)
    ...
    def evolve(self):
        """执行遗传算法进化"""
        print(f"[GA] Starting attack evolution with population size {self.pop_size}")

        for gen in range(self.generations):
            fitnesses = [self._fitness(ind) for ind in self.population]

            # 轮盘赌加权抽样：适应度越高被选中的概率越大（不是锦标赛选择）
            new_population = []
            ...
            # 找到当前代最佳个体。注意：fitnesses 是「进化前」这个种群的适应度，
            # 所以必须在 self.population 被 new_population 覆盖之前把个体取出来，
            # 否则会用旧适应度去索引新种群，报出来的最佳个体和适应度对不上。
...
def test_evolution():
    ...
    base_config = {"num_clients": 20, "model_shape": (10, 784), "num_features": 784, "num_classes": 10}

    evolution = AttackEvolution(FederatedSimulator, base_config, population_size=4, generations=3)
    best_attack = evolution.evolve()

    print("\n[DONE] Found optimal attack configuration:")
    print(best_attack)
