...
def classify_defense_result(
    baseline_acc: float, defended_acc: float, attack_events: int, num_classes: int
) -> Tuple[str, List[str]]:
    """Judge a defense on evidence rather than on "the accuracy did not drop".

    A model that never learned anything sits at chance level and therefore cannot drop, so
    ``acc_drop < 10`` used to report SUCCESS for runs where nothing happened at all.
    All three of these must hold before a defense may be called successful:

      1. the baseline learned something -- otherwise there is nothing to defend;
      2. at least one attack event was observed -- otherwise nothing was defended against;
      3. the defended model is no worse than the baseline, and above chance level.

    Returns the verdict and the reasons that kept it from being SUCCESS.
    """

    chance = 1.5 / num_classes
    reasons = []

    if baseline_acc < chance:
        reasons.append(
            f"baseline_acc={baseline_acc:.4f} is at chance level ({chance:.4f}), so the defense cannot be evaluated"
        )
    if attack_events == 0:
        reasons.append("attack_events=0, no attack occurred so nothing was defended against")
    if defended_acc < baseline_acc:
        reasons.append(f"defended_acc ({defended_acc:.4f}) < baseline_acc ({baseline_acc:.4f})")
    if defended_acc < chance:
        reasons.append(f"defended_acc={defended_acc:.4f} is still at chance level ({chance:.4f})")
...
def describe_outcome(baseline_acc: float, defended_acc: float, attack_events: int, num_classes: int) -> List[str]:
    """The OUTCOME block as lines.

    Kept apart from printing so the claim can be asserted on without standing up sockets.
    """

    verdict, reasons = classify_defense_result(baseline_acc, defended_acc, attack_events, num_classes)
    if verdict == "SUCCESS":
        return [" SUCCESS: Defense effectively mitigated attacks!"]

    lines = [f" {verdict}: this run does not prove the defense works"]
...
def run_adversarial_experiment(
    ...
    # Initialize server
    server = FederatedServer(host="127.0.0.1", port=5001, defense_type=defense_type, num_clients=num_clients)
    ...
    results = {"loss_history": [], "accuracy_history": [], "round_times": [], "attack_events": []}
    ...
    for round_idx in range(num_rounds):
        ...
        if round_idx > 0:
            ...
            if acc_drop > 5 and any(c.is_malicious for c in clients):
                results["attack_events"].append({"round": round_idx, "accuracy_drop": acc_drop})
    ...
    print("\n OUTCOME")
    for line in describe_outcome(peak_acc, final_acc, len(results["attack_events"]), num_classes):
        print(line)
