...
def main():
    ...
    # 模式选择
    parser.add_argument(
        "--mode", type=str, default="train", choices=["train", "tune", "benchmark", "ps", "worker"], help="运行模式"
    )
    ...
    parser.add_argument("--compression", type=float, default=0.1, help="压缩率")

    # 调优参数
    parser.add_argument("--tune-calls", type=int, default=20, help="贝叶斯优化调用次数")
    parser.add_argument("--tune-workers", type=int, default=4, help="并行调优Worker数")

    # 硬件配置
    parser.add_argument("--gpu-workers", type=int, default=2, help="GPU Worker数量")
    parser.add_argument("--cpu-workers", type=int, default=2, help="CPU Worker数量")

    # 分布式参数
    parser.add_argument("--bind", type=str, default="0.0.0.0", help="参数服务器绑定地址")
    parser.add_argument("--port", type=int, default=8888, help="端口")
    parser.add_argument("--server-ip", type=str, default="172.20.0.10", help="参数服务器IP")
    parser.add_argument("--worker-id", type=int, default=0, help="Worker ID")

    args = parser.parse_args()

    print(f"[START] Hermes Launcher starting in {args.mode} mode")
...
def run_parameter_server(args):
    """运行参数服务器模式"""
    print(f"\n[PS] Starting Parameter Server on {args.bind}:{args.port}")
    ...
    print(f"[OK] Parameter Server listening on {args.bind}:{args.port}")
    ...
    def handle_worker(conn, addr):
        ...
        print(f"[INFO] Worker connected from {addr}")
        ...
        try:
            while True:
                ...
                if message.startswith("get_params"):
                    ...
                    print(f"[STATS] Parameter updated: {params['value']:.4f}")
                elif message.startswith("heartbeat"):
                    # 更新心跳
                    workers[worker_id]["last_heartbeat"] = time.time()
        except Exception as e:
            print(f"[FAIL] Worker {worker_id} disconnected: {e}")
        finally:
        ...
        while True:
            ...
            for wid, info in workers.items():
                if now - info["last_heartbeat"] > 10:
                    ...
                    print(f"[DEAD] Worker {wid} timeout, removing from cluster")
...
def run_worker(args):
    ...
    while True:
        try:
            ...
            print(f"[OK] Worker {args.worker_id} connected to server")
            ...
            while True:
                ...
                if response.startswith("params:"):
                    ...
                    # 模拟计算梯度
                    grad = 2 * param - 4  # 目标: param = 2
                    print(f"[RETRY] Worker {args.worker_id} Iteration {iteration}: param={param:.4f}, grad={grad:.4f}")
            ...
            print(f"[FAIL] Worker {args.worker_id} connection lost: {e}")
...
def run_training(args):
    """运行训练模式"""
    print(f"\n[TRAIN] Training with lr={args.lr}, batch_size={args.batch_size}, compression={args.compression}")
...
def run_tuning(args):
    ...
    print(f"\n[TUNE] Hyperparameter Tuning with {args.tune_calls} trials")
    ...
    # 构造函数会把 worker 注册进调度器并打印初始化日志；此处暂不持有引用
    HardwareAwareScheduler(workers)

    # 定义搜索空间
    search_space = [("lr", "real", 1e-4, 1e-1), ("batch_size", "integer", 16, 128), ("compression", "real", 0.01, 0.5)]
    ...
    if best_loss < 0.05:
        ...
        print("[WARN] Hyperparameter tuning needs more iterations")
