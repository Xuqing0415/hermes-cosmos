...
class HardwareAwareScheduler:
    ...
    def __init__(self, workers: List[WorkerInfo]):
        ...
        print(f"[START] HardwareAwareScheduler initialized with {len(workers)} workers")
    ...
    def get_worker_stats(self) -> Dict:
        ...
        stats = {
            ...
            "avg_gpu_speed": sum(w.speed for w in self.workers if w.device_type == "gpu")
            / max(sum(1 for w in self.workers if w.device_type == "gpu"), 1),
            "avg_cpu_speed": sum(w.speed for w in self.workers if w.device_type == "cpu")
            / max(sum(1 for w in self.workers if w.device_type == "cpu"), 1),
