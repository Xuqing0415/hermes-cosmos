...
def print_results(results: Dict[str, List[Dict]], config: SimulationConfig):
    """打印模拟结果表格"""
    print("=" * 80)
    print("Hermes Unified Simulation Benchmark Results")
    print("=" * 80)
    ...
    for mode, data in results.items():
        ...
        for entry in data:
            print(
                f"{entry['num_workers']:<10} {entry['throughput']:<15.2f} "
...
def plot_results(results: Dict[str, List[Dict]], config: SimulationConfig):
    ...
    print("\n[STATS] 图表已保存到 simulation_results.png")
...
def main():
    ...
    parser.add_argument("--workers", type=str, default="1,2,4,8,16,32", help="List of worker counts (comma separated)")

    # Optimization parameters
    parser.add_argument(
        "--compression", type=float, default=1.0, help="Gradient compression ratio (0.1 equals 10 percent)"
    )
