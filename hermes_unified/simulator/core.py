...
class DistributedSimulator:
            ...
            for j in range(self.config.num_workers):
                if i != j:
                    ...
                    self.network_links[(i, j)] = NetworkLink(
                        src_id=i, dst_id=j, bandwidth_mb_s=base_bandwidth, latency=base_latency, congestion_level=0.0
                    )
    ...
    def get_network_link(self, src_id: int, dst_id: int) -> NetworkLink:
        ...
        return NetworkLink(
            src_id=src_id, dst_id=dst_id, bandwidth_mb_s=self.config.bandwidth_mb_s, latency=self.config.latency
        )
    ...
    def update_worker_dynamics(self):
        ...
        for worker in self.workers:
            if worker.alive and random.random() < self.config.worker_leave_rate:
                ...
                print(f"[HELLO] Worker {worker.worker_id} left at step {self.step_count}")
        ...
        if random.random() < self.config.worker_join_rate:
            ...
            print(f"[INFO] New worker {new_id} joined at step {self.step_count}")
    ...
    def compute_communication_time_gossip(self) -> float:
        ...
        for worker in alive_workers:
            ...
            neighbors = random.sample(
                [w for w in alive_workers if w.worker_id != worker.worker_id], min(k, len(alive_workers) - 1)
            )
    ...
    def _update_network_congestion(self):
        ...
        for (_src, _dst), link in self.network_links.items():
            # 根据并发流量更新拥塞
