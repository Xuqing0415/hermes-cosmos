...
try:
    import torch
except ImportError:  # torch 仅为可选依赖，仅 wrap_model 等训练场景需要
    torch = None
    ...
    async def async_save(self, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            response = await asyncio.get_event_loop().run_in_executor(
                None, requests.post, f"{self.config.server_url}/checkpoints", json=payload
            )
    ...
    async def async_load(self, checkpoint_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            if checkpoint_id is None:
                # 获取最新的checkpoint
                response = await asyncio.get_event_loop().run_in_executor(
                    None, requests.get, f"{self.config.server_url}/checkpoints/latest/{self.job_id}"
                )
                ...
                # 获取指定的checkpoint
                response = await asyncio.get_event_loop().run_in_executor(
                    None, requests.get, f"{self.config.server_url}/checkpoints/{checkpoint_id}"
                )
...
def enable_checkpointing(
    job_id: str, interval: int = 100, backend: str = "redis", server_url: str = "http://localhost:50052"
):
    """
    启用自动Checkpoint功能
    ...
    """
    config = CheckpointConfig(interval=interval, backend=backend, server_url=server_url)
...
if __name__ == "__main__":
    ...
    for step in range(500):
        ...
        # 手动保存
        cp_id = checkpointer.save({"model": model_state, "optimizer": optimizer_state, "step": step})
