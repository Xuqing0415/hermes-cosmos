...
def _line_count(field: str) -> int:
    """``git diff --numstat`` 里的一个计数。

    二进制文件的计数是 ``-``（git 不数行），``int("-")`` 会抛
    ``ValueError: invalid literal for int()``。这个异常不在
    ``except subprocess.CalledProcessError`` 的覆盖范围内，所以只要改动里有一个
    ``.bin``/``.png``/``.pyc``，整个变更分析就崩掉。计数缺失按 0 记。
    """
...
def parse_numstat(output: str) -> List[ChangedFile]:
    """把 ``git diff --numstat`` 的输出解析成 ``ChangedFile`` 列表。

    二进制文件计数为 0，但**文件名照常保留** —— 调用方仍然需要知道它变了。
    """
    ...
    for line in output.strip().split("\n"):
        ...
        additions = _line_count(parts[0])
        deletions = _line_count(parts[1])
        files.append(
            ChangedFile(
                filename=parts[2],
                additions=additions,
...
class ChangeAnalyzer:
    ...
    def _parse_function(self, node: ast.FunctionDef, filename: str, content: str) -> ChangedFunction:
    ...
    def analyze_local_changes(self) -> List[ChangedFunction]:
        ...
        try:
            ...
            changed_functions = []
            for cf in parse_numstat(result.stdout):
                if cf.filename.endswith(".py"):
                    functions = self._extract_changed_functions(cf)
                    changed_functions.extend(functions)
