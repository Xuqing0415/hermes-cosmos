...
class MLIRSymbolicExecutor:
    ...
    def _symbolic_execute_block(self, block_name: str, path_condition: Optional[z3.ExprRef]):
        ...
        # 注意：path_condition 是 z3 表达式，不能用 Python 真值判断（会抛 Z3Exception）
        path_key = f"{block_name}:{hash(str(path_condition))}" if path_condition is not None else f"{block_name}:none"
    ...
    def _execute_instruction(self, instr: Dict[str, Any], path_condition: Optional[z3.ExprRef]):
        ...
        if instr_type == "constant":
            ...
            lhs = self._get_z3_var(instr["lhs"], instr["dtype"])
            rhs = self._get_z3_var(instr["rhs"], instr["dtype"])
            # MLIR arith.addi 语义同 LLVM add：溢出按二进制回绕处理，不构成未定义行为，
            # 因此这里不做"可能溢出"告警（对符号输入必然误报）。
            ...
            if predicate == "eq":
                ...
                # z3py 对 BitVec 的 < / <= / > / >= 运算符即为带符号比较（无 SLT/SGE 函数）
                self.symbolic_values[instr["result"]] = lhs < rhs
            elif predicate == "sle":
                self.symbolic_values[instr["result"]] = lhs <= rhs
            elif predicate == "sgt":
    ...
    def _check_memref_bounds(self, memref_name: str, index: int, path_condition: Optional[z3.ExprRef]):
        ...
        if memref_type.shape[0] > 0 and (index < 0 or index >= memref_type.shape[0]):
            self.results.add_error(
                ...
                message=(
                    f"memref access index {index} out of bounds "
                    f"for memref {memref_name} with shape {memref_type.shape}"
                ),
            )
            return
        ...
        if terminator["type"] == "br":
            ...
            if Z3_AVAILABLE:
                ...
                if condition_var is not None:
                    true_condition = (
                        z3.And(path_condition, condition_var) if path_condition is not None else condition_var
                    )
                    false_condition = (
                        z3.And(path_condition, z3.Not(condition_var))
                        if path_condition is not None
                        else z3.Not(condition_var)
                    )
