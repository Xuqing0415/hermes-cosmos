...
class TheoremProver:
    ...
    def _prove_z3(self, smt_expression: str) -> ProofResult:
        ...
        try:
            ...
            # 标准定理证明语义：证 P 成立 = 检查 ¬P 不可满足(unsat)。
            # ¬P 可满足时给出反例模型 => DISPROVEN。
            if smt_expression.startswith("(assert"):
                parsed = z3.parse_smt2_string(smt_expression)
            else:
                parsed = z3.parse_smt2_string(f"(assert {smt_expression})")

            claim = z3.And(parsed) if len(parsed) > 1 else parsed[0]
            self._solver.add(z3.Not(claim))
            ...
            if result == z3.unsat:
                return ProofResult(
                    ...
                    certificate="Z3 proof completed: negation is unsatisfiable",
                )
            elif result == z3.sat:
                ...
                return ProofResult(status=ProofStatus.DISPROVEN, smt_expression=smt_expression, model=model_dict)
            else:
                return ProofResult(
                    status=ProofStatus.UNKNOWN, smt_expression=smt_expression, error_message="Solver returned unknown"
                )

        except Exception as e:
            logger.error("Z3 proving failed", error=str(e))
            return ProofResult(status=ProofStatus.UNKNOWN, smt_expression=smt_expression, error_message=str(e))
    ...
    def _prove_mock(self, smt_expression: str) -> ProofResult:
        ...
        if "(assert true)" in smt_lower or smt_lower == "true":
            return ProofResult(
                status=ProofStatus.PROVEN,
                smt_expression=smt_expression,
                certificate="Mock proof: true is trivially valid",
            )

        if "(assert false)" in smt_lower or smt_lower == "false":
            return ProofResult(
                status=ProofStatus.DISPROVEN,
                smt_expression=smt_expression,
                certificate="Mock proof: false is never provable",
            )

        if "b == 0" in smt_lower or "(= b 0)" in smt_lower:
            return ProofResult(status=ProofStatus.DISPROVEN, smt_expression=smt_expression, model={"b": "0"})

        return ProofResult(status=ProofStatus.UNKNOWN, smt_expression=smt_expression)
