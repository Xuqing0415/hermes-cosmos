...
from hermes.agent_civilization.task_board import TaskBoard
from hermes.llm_requirement.self_audit import AuditIssue, AuditIssueType, AuditResult, IssueSeverity, SelfAuditor
...
from hermes.self_refactor.smell_detector import SmellDetector, SmellInstance, SmellType
...
class FeatureExtractor:
        ...
        if task_stats.get("failed", 0) > 0:
            ...
            if failure_rate > 0.1:
                candidate = FeatureCandidate(
                    ...
                    suggested_actions=["分析失败任务的共同特征", "优化任务调度策略", "增加任务重试机制"],
                )
                candidates.append(candidate)
        ...
        for task_type, stats in type_stats.items():
            if stats.get("success_rate", 1.0) < 0.8:
                candidate = FeatureCandidate(
                    ...
                    suggested_actions=[f"分析 {task_type} 任务失败原因", f"改进 {task_type} 任务处理逻辑"],
                )
                candidates.append(candidate)
    ...
    def _extract_from_proof_failures(self, existing_data: Optional[Dict[str, Any]]) -> List[FeatureCandidate]:
        ...
        if disproven_count > 0:
            candidate = FeatureCandidate(
                ...
                suggested_actions=["分析证伪的反例", "修复相关代码缺陷", "重新验证修复后的属性"],
            )
            candidates.append(candidate)
        ...
        if unknown_count > disproven_count:
            candidate = FeatureCandidate(
                ...
                metadata={"unknown_count": unknown_count, "timeout_count": timeout_count},
                related_issues=["证明超时/未知"],
                suggested_actions=["优化证明策略", "增加证明超时时间", "引入更强的证明引擎"],
            )
            candidates.append(candidate)
    ...
    def _get_smell_actions(self, smell: SmellInstance) -> List[str]:
        actions = {
            SmellType.CYCLE_DEPENDENCY: ["识别循环依赖路径", "引入中间层解耦", "重新组织模块结构"],
            SmellType.DUPLICATE_CODE: ["提取公共函数/类", "创建共享模块", "消除重复逻辑"],
            SmellType.GOD_MODULE: ["识别职责边界", "拆分为多个小模块", "应用单一职责原则"],
            SmellType.LONG_FUNCTION: ["提取子函数", "简化条件逻辑", "应用策略模式"],
            SmellType.DATA_CLUMP: ["创建数据类封装", "使用对象替代参数列表", "减少参数传递"],
            SmellType.SHOTGUN_SURGERY: ["集中常量定义", "使用配置文件", "创建单一数据源"],
            SmellType.FEATURE_ENVY: ["移动方法到正确的类", "重新设计类职责", "应用迪米特法则"],
        }
