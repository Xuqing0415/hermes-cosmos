...
from hermes.neural_symbolic.types import GeneratedTest, PathCondition, ProofResult, ProofStatus, ProofTarget
...
class TestTheoremProver:
    ...
    def test_prove_false(self):
        ...
        # false 不是定理：证明器应给出 DISPROVEN（反驳）而非 PROVEN
...
class TestProofCertificate:
    ...
    def test_add_proof_result(self):
        cert = ProofCertificate()
        result = ProofResult(status=ProofStatus.PROVEN, smt_expression="(not (= b 0))", certificate="Test proof")
    ...
    def test_add_test(self):
        cert = ProofCertificate()
        test = GeneratedTest(id="test_1", function_name="divide", inputs={"a": 10, "b": 2})
    ...
    def test_to_dict(self):
        cert = ProofCertificate()
        result = ProofResult(status=ProofStatus.PROVEN, smt_expression="(not (= b 0))")
        cert.add_proof_result(result)
...
class TestNeuralProofTargetPredictor:
    ...
    def test_predict(self):
        ...
        targets = [
            ProofTarget(
                id="test_1", function_name="divide", path_condition=PathCondition("(not (= b 0))"), risk_score=0.8
            )
        ]
    ...
    def test_predict_top_targets(self):
        ...
        targets = [
            ...
            ProofTarget(id="test_2", function_name="add", path_condition=PathCondition("(> x 0)"), risk_score=0.2),
        ]
...
class TestProofGuidedTestGenerator:
    ...
    def test_generate_from_proven(self):
        ...
        result = ProofResult(status=ProofStatus.PROVEN, smt_expression="(not (= b 0))")
        target = ProofTarget(id="test_1", function_name="divide", path_condition=PathCondition("(not (= b 0))"))
    ...
    def test_generate_from_disproven(self):
        generator = ProofGuidedTestGenerator()
        result = ProofResult(status=ProofStatus.DISPROVEN, smt_expression="(not (= b 0))", model={"b": "0"})
        target = ProofTarget(id="test_1", function_name="divide", path_condition=PathCondition("(not (= b 0))"))
    ...
    def test_generate_test_code(self):
        generator = ProofGuidedTestGenerator()
        test = GeneratedTest(id="test_1", function_name="divide", inputs={"a": 10, "b": 2}, assertion="assert b != 0")
    ...
    def test_generate_test_module(self):
        generator = ProofGuidedTestGenerator()
        tests = [
            GeneratedTest(id="test_1", function_name="divide", inputs={"a": 10, "b": 2}, assertion="assert b != 0")
        ]
